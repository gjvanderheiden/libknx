...
ByteBufferReader::ByteBufferReader(const std::span<const byte> data)
    : data{data} {}

std::uint32_t ByteBufferReader::readUint32() {
  std::uint32_t value{0};
  memcpy(&value, data.subspan(index, sizeof(std::uint32_t)).data(),
         sizeof(std::uint32_t));
...
std::span<const byte> ByteBufferReader::readByteSpan(const int numberOfBytes) {
  std::span<const byte> const span = data.subspan(index, numberOfBytes);
  index += numberOfBytes;
  return span;
}

void ByteBufferReader::copyToSpan(std::span<byte> destination) {
  memcpy(destination.data(), data.subspan(index, destination.size()).data(),
         destination.size());
