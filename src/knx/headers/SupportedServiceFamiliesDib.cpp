...
SupportedServiceFamiliesDib::SupportedServiceFamiliesDib(std::vector<ServiceFamily> serviceFamilies)
    : KnxStructure(SupportedServiceFamiliesDib::TYPE), serviceFamilies{serviceFamilies} {}
...
SupportedServiceFamiliesDib SupportedServiceFamiliesDib::parse(ByteBufferReader &reader) {
  auto [length, type] = KnxStructure::parse(reader);
  SupportedServiceFamiliesDib result{};
  auto size = length / 2;
  result.serviceFamilies.reserve(size);
  for(int i = 0; i < size; i++) {
    result.serviceFamilies.emplace_back(toEnum(reader.readUint8()), reader.readUint8());
  }
  return result;
}

std::vector<ServiceFamily> SupportedServiceFamiliesDib::getServiceFamilies() const {
